...
async function callGemini(prompt: string, systemPrompt?: string): Promise<string> {
  if (!config.enableAI || !config.gcpProjectId) {
    return "AI is currently disabled. This is a mock response for development.";
  }

  try {
    const client = getAIClient();
    const model = `projects/${config.gcpProjectId}/locations/${config.gcpLocation}/publishers/google/models/${config.geminiModel}`;
