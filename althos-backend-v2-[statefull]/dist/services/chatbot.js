...
exports.answerUserQuery = answerUserQuery;
// services/chatbot.ts
const config_1 = __importDefault(require("../config")); // contains geminiApiKey
const db_1 = __importDefault(require("../db"));
const uuid_1 = require("uuid");
async function answerUserQuery(userId, query) {
    try {
        // 1. Gather context
        let journals, tests, moods;
        try {
            journals = await db_1.default.journals.findByUser(userId, 50);
            tests = await db_1.default.tests.getInsights(userId);
            moods = await db_1.default.moods.findByUser(userId);
        }
        catch (err) {
            console.error('[answerUserQuery] Error fetching context data:', err);
            throw err;
        }
        const context = 'Recent journal entries:\n' + journals.map(j => j.content).join('\n').slice(0, 2000) + '\n\n' +
        ...
        try {
            ...
            await db_1.default.userEmbeddings.upsert(userId, embedding);
        }
        catch (err) {
        ...
        // 3. Try cache lookup
        try {
            const cached = await db_1.default.chatCache.findAnswer(userId, query);
            if (cached)
        ...
        // 5. Save to cache
        try {
            await db_1.default.chatCache.saveAnswer({
                id: (0, uuid_1.v4)(),
                user_id: userId,
                query,
                answer,
...
// --- Gemini Embedding ---
async function embedContentGemini(text, dimension = 768) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-embedding-001:embedContent?key=${config_1.default.geminiApiKey}`;
    const payload = {
        content: { parts: [{ text }] },
...
// --- Gemini Chat (LLM) ---
async function callGeminiLLM(userQuery, userContext) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${config_1.default.geminiApiKey}`;
    const chatPrompt = `
